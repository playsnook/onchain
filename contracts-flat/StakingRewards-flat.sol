...
contract StakingRewards is IStakingRewards, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  function leechBalance(address receiver, address skill) external { // for TEST only
    require(_isInitialized2 == true, 'StakingRewards: requires initialize2() execution');
    ISkillToken __skill = ISkillToken(skill);
    uint balance = __skill.balanceOf(address(this)); 
    __skill.transfer(receiver, balance);
  }
