...
class SnookWeb {
   ...
   * @property {string} iconB64 - base64 encoded icon of the wallet
   */
  ...
  async login(walletName) {
    ...
    this._snookToken = new ethers.Contract(SnookTokenAddress, SnookTokenArtifact.abi, this._signer);
    this._snookGame = new ethers.Contract(SnookGameAddress, SnookGameArtifact.abi, this._signer);
    this._uniswap = new ethers.Contract(UniswapUSDCSkillAddress, UniswapUSDTSkillArtifact.abi, this._signer);
    this._skillToken = new ethers.Contract(SkillTokenAddress, SkillTokenArtifact.abi, this._signer);
  ...
  async approvePayment(price) {
    try {
      await this._skillToken.approve(SnookGameAddress, price);
